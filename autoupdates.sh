...
sed -i.bak 's+//      "${distro_id}:${distro_codename}-updates";+        "${distro_id}:${distro_codename}-updates";+g' /etc/apt/apt.conf.d/50unattended-upgrades

# add in Allowed-Origins: "Google LLC:stable";
sed -i.bak 's+//      "${distro_id}:${distro_codename}-backports";+&\n        "Google LLC:stable";+' /etc/apt/apt.conf.d/50unattended-upgrades
